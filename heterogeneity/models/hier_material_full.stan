...
parameters {
  ...
  vector<lower=0>[N_materials] sigma_locations;
  vector[N_locations] z_locations;
...
transformed parameters {
  ...
  vector[N_locations] mu_locations = mu_holders[holder_ids] + sigma_holders[material_ids][sample_ids][holder_ids].*z_locations;
  vector[N_ids] mu_ids = mu_locations[location_ids] + sigma_locations[material_ids][sample_ids][holder_ids][location_ids].*z_measurements;
}
...
model {
  ...
  sigma_locations ~ std_normal();
  z_samples ~ std_normal();
  z_holders ~ std_normal();
  z_locations ~ std_normal();
