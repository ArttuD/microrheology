...
model {
  ...
  y[train_ids] ~ normal(mu_ids[material_ids][sample_ids][holder_ids][location_ids][track_ids][train_ids],
               sigma_common[track_ids][train_ids]);
}
...
generated quantities {
 ...
 real y_hat2[N_ids];
 for (i in 1:N) {
    log_likelihood[i] = normal_lpdf(y[i] | mu_ids[material_ids][sample_ids][holder_ids][location_ids][track_ids[i]],sigma_common[track_ids[i]]);
 } 
 y_hat = normal_rng(mu_ids[material_ids][sample_ids][holder_ids][location_ids][track_ids],sigma_common[track_ids]);
 y_hat2 = normal_rng(mu_ids[material_ids][sample_ids][holder_ids][location_ids],sigma_common);
}